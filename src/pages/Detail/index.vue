...
export default {
  ...
  methods: {
    ...
    inputSkuNum(event) {
      ...
      //含非数字内容*1=NaN
      if (isNaN(num * 1) || num < 1) this.skuNum = 1;
      else this.skuNum = parseInt(num);
    },
    async updateShopCar() {
      try {
  ...
  mounted() {
    this.$store.dispatch("detail/getGoodsInfo", this.$route.query.id);
  },
};
</script>
